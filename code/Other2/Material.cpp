...
bool Material::EndLoad()
{
    ...
    if (root.Contains("textures"))
    {
        ...
        for (auto it = jsonTextures.Begin(); it != jsonTextures.End(); ++it)
            SetTexture(it->first.ToInt(), cache->LoadResource<Texture2>(it->second.GetString()));
    }
...
void Material::SetTexture(size_t index, Texture2* texture)
{
...
Texture2* Material::GetTexture(size_t index) const
{
