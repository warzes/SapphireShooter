...
void GeometryNode::SetGeometry(size_t index, Geometry* geometry)
{
    if (!geometry)
    {
        SE_LOG_ERROR("Can not assign null geometry");
        return;
    }
    ...
    if (index < batches.Size())
        ...
        SE_LOG_ERROR("Out of bounds batch index " + std::to_string((int)index) + " for setting geometry");
}
...
void GeometryNode::SetMaterial(size_t index, Material* material)
{
    if (index < batches.Size())
    {
        ...
        SE_LOG_ERROR("Out of bounds batch index " + std::to_string((int)index) + " for setting material");
}
