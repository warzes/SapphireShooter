...
bool Graphics::CreateD3DDevice(int multisample_)
{
    ...
    if (!impl->device)
    {
        ...
        {
            SE_LOG_ERROR("Failed to create D3D11 device");
            return false;
        }
    }
    ...
    {
        SE_LOG_ERROR("Failed to create D3D11 swap chain");
        return false;
    }
}
...
bool Graphics::UpdateSwapChain(int width, int height)
{
    ...
    {
        SE_LOG_ERROR("Failed to get backbuffer texture");
        success = false;
    }
    ...
    {
        SE_LOG_ERROR("Failed to create backbuffer depth-stencil texture");
        success = false;
    }
        ...
        if (newInputLayout != inputLayout)
        {
            ...
            else
            {
                ...
                if (d3dInputLayout)
                {
                    ...
                    SE_LOG_ERROR("Failed to create input layout");
            }
        }
    }
