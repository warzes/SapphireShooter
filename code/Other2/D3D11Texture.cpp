...
bool Texture2::Define(TextureType type_, ResourceUsage usage_, const IntVector2& size_, ImageFormat format_, size_t numLevels_, const ImageLevel* initialData)
{
    ...
    {
        SE_LOG_ERROR("Only 2D textures and cube maps supported for now");
        return false;
    }
    if (format_ > FMT_DXT5)
    {
        SE_LOG_ERROR("ETC1 and PVRTC formats are unsupported");
        return false;
    }
    if (type_ == TEX_CUBE && size_.x != size_.y)
    {
        SE_LOG_ERROR("Cube map must have square dimensions");
        return false;
    }
    ...
    if (graphics && graphics->IsInitialized())
    {
        ...
        if (!texture)
        {
            ...
            SE_LOG_ERROR("Failed to create texture");
            return false;
        }
        else
        {
        ...
        if (IsRenderTarget())
        {
            ...
            if (!renderTargetView)
            {
                SE_LOG_ERROR("Failed to create rendertarget view for texture");
            }
        }
        else if (IsDepthStencil())
        {
            ...
            if (!renderTargetView)
            {
                SE_LOG_ERROR("Failed to create depth-stencil view for texture");
            }
        }
        ...
        if (!resourceView)
        {
            SE_LOG_ERROR("Failed to create shader resource view for texture");
        }
    }
...
bool Texture2::DefineSampler(TextureFilterMode filter_, TextureAddressMode u, TextureAddressMode v, TextureAddressMode w, unsigned maxAnisotropy_, float minLod_, float maxLod_, const Color& borderColor_)
{
    ...
    if (graphics && graphics->IsInitialized())
    {
        ...
        if (!sampler)
        {
            SE_LOG_ERROR("Failed to create sampler state");
            return false;
        }
        else
...
bool Texture2::SetData(size_t face, size_t level, IntRect rect, const ImageLevel& data)
{
    ...
    if (texture)
    {
        ...
        {
            SE_LOG_ERROR("Can not update immutable texture");
            return false;
        }
        if (face >= NumFaces())
        {
            SE_LOG_ERROR("Face to update out of bounds");
            return false;
        }
        if (level >= numLevels)
        {
            SE_LOG_ERROR("Mipmap level to update out of bounds");
            return false;
        }
        ...
        {
            SE_LOG_ERROR("Texture update region is outside level");
            return false;
        }
        ...
        if (usage == USAGE_DYNAMIC)
        {
            ...
            if (!pixelByteSize)
            {
                SE_LOG_ERROR("Updating dynamic compressed texture is not supported");
                return false;
            }
            ...
            {
                SE_LOG_ERROR("Failed to map texture for update");
                return false;
            }
        }
        else
