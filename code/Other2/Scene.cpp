...
void Scene::Save(Stream& dest)
{
	...
	SE_LOG(("Saving scene to " + dest.Name()).CString());
...
bool Scene::Load(Stream& source)
{
	...
	SE_LOG(("Loading scene from " + source.Name()).CString());

	String fileId = source.ReadFileID();
	if (fileId != "SCNE")
	{
		SE_LOG_ERROR("File is not a binary scene file");
		return false;
	}
	...
	{
		SE_LOG_ERROR("Mismatching type of scene root node in scene file");
		return false;
	}
...
void Scene::DefineLayer(unsigned char index, const String& name)
{
	if (index >= 32)
	{
		SE_LOG_ERROR("Can not define more than 32 layers");
		return;
	}
