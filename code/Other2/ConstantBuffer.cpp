...
bool ConstantBuffer::LoadJSON(const JSONValue& source)
{
	...
	for (size_t i = 0; i < jsonConstants.Size(); ++i)
	{
		...
		{
			SE_LOG_ERROR(("Unknown element type" + type + " in constant buffer JSON").CString());
			break;
		}
...
bool ConstantBuffer::Define(ResourceUsage usage_, size_t numConstants, const Constant* srcConstants)
{
	...
	if (!numConstants)
	{
		SE_LOG_ERROR("Can not define constant buffer with no constants");
		return false;
	}
	if (usage_ == USAGE_RENDERTARGET)
	{
		SE_LOG_ERROR("Rendertarget usage is illegal for constant buffers");
		return false;
	}
	...
	while (numConstants--)
	{
		...
		{
			SE_LOG_ERROR("UBYTE4 type is not supported in constant buffers");
			constants.Clear();
			byteSize = 0;
			return false;
		}
