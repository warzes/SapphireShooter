...
bool Model::BeginLoad(Stream& source)
{
    ...
    if (numMorphs)
    {
        SE_LOG_ERROR("Models with vertex morphs are not supported for now");
        return false;
    }
    ...
    for (size_t i = 0; i < geomDescs.Size(); ++i)
    {
        ...
        for (size_t j = 0; j < geomDescs[i].Size(); ++j)
        {
            ...
            if (geomDesc.vbRef < vbs.Size())
                ...
                SE_LOG_ERROR(("Out of range vertex buffer reference in " + Name()).CString());

            if (geomDesc.ibRef < ibs.Size())
                geom->indexBuffer = ibs[geomDesc.ibRef];
            else
                SE_LOG_ERROR(("Out of range index buffer reference in " + Name()).CString());
...
void Model::SetNumLodLevels(size_t index, size_t num)
{
    ...
    {
        SE_LOG_ERROR("Out of bounds geometry index for setting number of LOD levels");
        return;
    }
