...
void ProjectAndMergeEdge(Vector3 v0, Vector3 v1, Rect& rect, const Matrix4& projection)
{
	...
	if (v0.z < RoundingError && v1.z < RoundingError)
		return;

	// Check if need to clip one of the vertices
	if (v1.z < RoundingError)
		v1 = ClipEdgeZ(v1, v0, RoundingError);
	else if (v0.z < RoundingError)
		v0 = ClipEdgeZ(v0, v1, RoundingError);
