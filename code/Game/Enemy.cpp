...
void Enemy::Draw()
{
	if (!m_dead)
	{
		...
		//Renderer::Get().GetComponent(enemyId).GetShaderComponent()->Bind();

		// Check if the enemy is taking damage, if so, make the enemy flash red
		//Renderer::Get().GetComponent(enemyId).GetShaderComponent()->SetBool("damaged", m_takingDamage);
		...
		if (m_droneActive)
		{
			...
			//Renderer::Get().GetComponent(enemyDroneId).SetTransform(m_dronePos, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.25f, 0.25f, 0.25f));
			//Renderer::Get().GetComponent(enemyDroneId).Draw(m_camera);

			// Check if the player is colliding with the drone
			if (PhysicsManager::Get().PointInSphere(m_camera, m_dronePos, 2.0f))
			{
				// Inflict damage on player
				PhysicsManager::Get().OnPlayerHit(m_attackDamage);
			}
		}
	}
	else
...
void Enemy::DrawShockwave()
{
	...
	if (m_droneSelfDestruct)
	{
		...
		if (m_blastRadius < 7.0f)
		{
			...
			//Renderer::Get().GetComponent(enemyDroneBlastId).SetTransform(m_oldPlayerPos, glm::vec3(m_blastRadius * 20, m_blastRadius * 20, m_blastRadius * 20), glm::vec3(m_blastRadius));
			//Renderer::Get().GetComponent(enemyDroneBlastId).Draw(m_camera);

			// Check if enemy can damage and player is caught within the growing blast
			if (m_damageToken && PhysicsManager::Get().PointInSphere(m_camera, m_oldPlayerPos, (m_blastRadius * 4)))
			{
				// Inflict damage on player
				PhysicsManager::Get().OnPlayerHit(m_attackDamage);
	...
	if (!m_dead)
	{
		...
		if (m_takingDamage)
		{
			...
			else
			{
				...
				int dashSidewaysChance = RandomUtils::Get().RandomNumBetween1And100();
				...
				if (dashSidewaysChance > 97)
				{
					...
					int dashSide = RandomUtils::Get().RandomNumBetween1And100();
		...
		{
			m_shootDuration += RandomUtils::Get().RandomNumBetweenTwo(0.1f, 0.5f) * dt;
		}
...
void Enemy::Respawn()
{
	if (m_canRespawn)
	{
		...
		if (m_respawnTimer >= 15.0f)
		{
			...
			m_pos = glm::vec3(RandomUtils::Get().RandomNumBetweenTwo(50.0f, 520.0f), 0.0f, RandomUtils::Get().RandomNumBetweenTwo(0.0f, 650.0f));
		}
	}
}